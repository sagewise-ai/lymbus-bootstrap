      ...
      <div className="d-grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          <div key={i} className="bg-card p-4 border border-brand-border d-d-flex align-align-items-center gap-4 hover:border-brand-blue/30 transition-colors" style={{'borderRadius':'16px'}}>
            ...
            <div>
              <p className="fw-bold text-brand-gray text-uppercase tracking-widest leading-none mb-1" style={{'fontSize':'10px'}}>{item.title}</p>
              <p className="text-xs fw-bold text-brand-dark leading-tight">{item.desc}</p>
            </div>
          </div>
        ))}
      </div>

      <div className="d-grid grid-cols-1 xl:grid-cols-2 gap-4 lg:gap-6">
