        ...
        <div className="flex-fill space-y-4">
          <div className="d-flex align-items-center justify-content-between">
            <div className="d-flex align-items-center gap-2">
              ...
              <span className="fw-bold text-uppercase tracking-widest text-brand-gray/60" style={{'fontSize':'10px'}}>{element.label}</span>
              {element.properties?.weightage !== undefined && (
                <span className="px-2 py-0.5 rounded-circle bg-brand-blue/10 text-brand-blue fw-bold text-uppercase tracking-widest border border-brand-blue/10" style={{'fontSize':'8px'}}>
                  W: {element.properties.weightage}
                </span>
              )}
              {element.required && (
                <span className="text-red-500 text-xs fw-bold">*</span>
              )}
            </div>
          ...
          <div className="space-y-3">
            <input 
              ...
              className="w-100 text-lg fw-bold text-brand-dark bg-transparent border-bottom border-transparent focus:border-brand-blue/20 outline-none pb-1 transition-all"
            />
                ...
                <div className="d-grid grid-cols-1 gap-2">
                  {element.properties.options.map((opt: string, i: number) => (
                    <div key={i} className="d-flex align-items-center gap-3 px-4 py-2 bg-brand-bg/30 border border-brand-border/50 rounded-3">
                      ...
                      <span className="text-xs fw-medium text-brand-gray/60">{opt}</span>
                    </div>
                  ))}
                </div>
              ) : (
    ...
    <div className="flex-fill w-100 d-flex flex-column align-items-center p-4 lg:p-8 overflow-hidden h-100">
      ...
      <div className="d-flex align-items-center gap-1 bg-white border border-brand-border p-1 rounded-3 mb-6 shadow-sm flex-shrink-0">
        {[
          ...
          >
            <device.icon size={16} />
            <span className="d-none sm:inline">{device.label}</span>
          </button>
        ))}
      </div>
        ...
        <div className={cn(
          ...
          <div className="max-w-3xl mx-auto space-y-8">
            ...
            <div className="pt-8 pb-12">
              <button className="w-100 py-4 bg-brand-blue text-white fw-bold text-sm text-uppercase tracking-widest shadow-xl shadow-brand-blue/20 hover:bg-brand-blue/90 hover:shadow-brand-blue/30 transition-all active:scale-[0.99]" style={{'borderRadius':'20px'}}>
                Submit Feedback
              </button>
      ...
      >
        <div className="w-100 max-w-3xl d-flex flex-column align-items-center">
          ...
          {activeTab === 'preview' ? (
            ...
            <div className="w-100 space-y-6" onClick={e => e.stopPropagation()}>
              {/* Form Header Card */}
              <div className="bg-white border border-brand-border p-8 shadow-sm" style={{'borderRadius':'32px'}}>
                 <input 
                   type="text" 
                   value={formName}
                   onChange={(e) => setFormName(e.target.value)}
                   placeholder="Untitled form"
                   className="w-100 text-2xl fw-bold text-brand-dark bg-transparent border-none outline-none placeholder:text-brand-gray/20"
                 />
                 <div className="w-12 h-1 bg-brand-blue rounded-circle mt-4" />
              </div>
              ...
              >
                {canvasElements.length === 0 ? (
                  <div className="d-flex flex-column align-items-center text-center p-12">
                    ...
                    <p className="text-xs text-brand-gray fw-medium max-w-xs leading-relaxed">
                ...
                <AnimatePresence mode="wait">
                  {draftElement ? (
                          ...
                          <div className="d-flex align-items-center justify-content-between p-4 bg-brand-bg/50 rounded-3 border border-brand-border">
                            ...
                            <div className="d-flex align-items-center gap-2 bg-white rounded-3 border border-brand-border p-1">
                              <input 
                                ...
                                className="w-12 text-center text-xs fw-bold text-brand-blue outline-none"
                              />
                            </div>
                          </div>
                        </div>
                          ...
                          <div className="p-4 bg-brand-bg/50 rounded-3 border border-brand-border space-y-3">
                            ...
                            <div className="position-relative">
                              <select 
                                ...
                                className="w-100 bg-white border border-brand-border rounded-3 px-4 py-2.5 text-xs fw-bold text-brand-dark focus:ring-1 focus:ring-brand-blue/20 outline-none appearance-none transition-all cursor-pointer"
                              >
                                {categories.map(cat => (
                            ...
                            <div className="pt-2 border-top border-brand-border/50">
                              <button 
                                onClick={() => {
                    ...
                    <div className="d-flex flex-column align-items-center justify-content-center h-100 text-center py-20">
                      ...
                      <p className="text-xs text-brand-gray leading-relaxed fw-medium">
              ...
              <div className="p-4 space-y-3 flex-shrink-0">
                <div className="position-relative">
                  ...
                  <input 
                    ...
                    className="w-100 bg-brand-bg/50 border border-brand-border rounded-3 pl-9 pr-4 py-2.5 text-xs fw-medium focus:bg-white focus:ring-1 focus:ring-brand-blue/20 transition-all outline-none"
                  />
                </div>
                    ...
                    filteredQuestions.length > 0 ? (
                      filteredQuestions.map((q, idx) => (
                        ...
                        >
                          ...
                          <p className="text-xs fw-bold text-brand-dark leading-relaxed line-clamp-2">{q.text}</p>
                        </Motion.button>
                      ))
                    ) : (
                    ...
                    filteredFiles.length > 0 ? (
                      filteredFiles.map((f, idx) => (
                        ...
                        >
                          ...
                          <div className="p-1.5 rounded-3 bg-brand-bg text-brand-gray group-hover:text-brand-blue transition-colors opacity-0 group-hover:opacity-100">
                             <Plus size={12} />
                          </div>
                        </Motion.button>
                      ))
