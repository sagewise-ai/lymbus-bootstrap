          ...
          !isAlwaysVisible && "scale-90 group-hover/toolbar:scale-100 border-brand-blue/20"
           ...
           {questionTypes.map(type => (
             <button 
               ...
               className="w-9 h-9 bg-brand-bg hover:bg-brand-blue hover:text-white rounded-circle d-flex align-items-center justify-content-center text-brand-gray transition-all border border-brand-border group position-relative"
               title={type.label}
             >
               <type.icon size={16} />
               <span className="position-absolute -top-10 left-1/2 -translate-x-1/2 bg-brand-dark text-white fw-bold px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pe-none" style={{'fontSize':'8px'}}>
...
export const FormBuilderClassic = ({ onBack }: { onBack: () => void }) => {
  ...
  const renderPreview = () => (
    <div className="flex-fill w-100 d-flex align-items-center justify-content-center p-4 lg:p-8 overflow-hidden animate-in zoom-in-95 duration-500">
      <div className="position-relative h-100 w-100 bg-brand-dark p-4 shadow-2xl border-brand-dark overflow-hidden d-flex flex-column" style={{'maxHeight':'820px', 'maxWidth':'380px', 'borderRadius':'60px', 'borderColor':'8px'}}>
        {/* Notch */}
        <div className="position-absolute top-0 left-1/2 -translate-x-1/2 w-40 h-7 bg-brand-dark z-20 d-flex align-items-center justify-content-center">
          <div className="w-12 h-1 bg-white/10 rounded-circle" />
        </div>

        {/* Screen */}
        <div className="flex-fill bg-white overflow-y-auto no-scrollbar pt-10 pb-10 px-6" style={{'borderRadius':'40px'}}>
          <div className="space-y-8">
              ...
              {questions.map((q, i) => (
                <div key={q.id} className="space-y-4 animate-in fade-in slide-in-from-bottom-4" style={{ animationDelay: `${i * 100}ms` }}>
                  <div className="d-flex align-items-start gap-3">
                    <span className="w-6 h-6 rounded-3 bg-brand-bg d-flex align-items-center justify-content-center fw-bold text-brand-blue flex-shrink-0 mt-1" style={{'fontSize':'10px'}}>{i + 1}</span>
                    ...
                    <div className="space-y-3">
                      <div className="d-flex flex-wrap gap-1 justify-content-center">
                        {Array.from({ length: (q.properties?.range || 10) + 1 }).map((_, n) => (
                          <button key={n} className="w-7 h-7 rounded-3 border border-brand-border d-flex align-items-center justify-content-center fw-bold hover:bg-brand-blue hover:text-white hover:border-brand-blue transition-all" style={{'fontSize':'10px'}}>
                            {n}
                          </button>
                        ))}
                      </div>
                      <div className="d-flex justify-content-between fw-bold text-uppercase tracking-widest text-brand-gray px-1" style={{'fontSize':'8px'}}>
                  ...
                  {q.type === 'rating' && (
                    <div className="d-flex align-items-center justify-content-between px-2">
                      {Array.from({ length: q.properties?.range || 5 }).map((_, n) => (
                        <button key={n} className="w-10 h-10 rounded-3 bg-brand-bg border border-brand-border d-flex align-items-center justify-content-center text-brand-gray/30 hover:bg-brand-blue/5 hover:border-brand-blue/20 transition-all">
                          <Star size={20} />
                        </button>
                      ))}
                    </div>
                    ...
                    <div className="space-y-4">
                      <div className="d-flex align-items-center justify-content-between gap-1.5">
                        {Array.from({ length: q.properties?.range || 5 }).map((_, n) => (
                          <button 
                            key={n} 
                            className="flex-fill h-12 rounded-3 bg-brand-bg border border-brand-border d-flex flex-column align-items-center justify-content-center gap-1 hover:bg-brand-blue/5 hover:border-brand-blue transition-all group"
                          >
                            <span className="text-xs fw-bold text-brand-dark group-hover:text-brand-blue">{n + 1}</span>
                          </button>
                        ))}
                      </div>
                      <div className="d-flex justify-content-between px-1 gap-4">
                    ...
                    <div className="position-relative">
                      {q.type === 'text' ? (
                        <input 
                          ...
                          className="w-100 bg-brand-bg/50 border border-brand-border rounded-3 px-4 py-3 text-sm focus:outline-none placeholder:text-brand-gray/30"
                          readOnly
                        />
                      ) : (
                        <textarea 
                          placeholder="Detailed feedback..."
                          className="w-100 h-32 bg-brand-bg/50 border border-brand-border rounded-2xl p-4 text-sm focus:outline-none placeholder:text-brand-gray/30 resize-none"
                          readOnly
                        />
                      )}
                    </div>
                  ...
                  {(q.type === 'radio' || q.type === 'checkbox') && (
                    <div className="space-y-2">
                      {(q.options || ['Option 1', 'Option 2']).map((opt, idx) => (
                        <button key={idx} className="w-100 p-4 rounded-2xl bg-white border border-brand-border d-flex align-items-center justify-content-between group hover:border-brand-blue transition-all text-start">
            ...
            <div className="d-flex flex-column align-items-center gap-2 py-8 opacity-40">
        ...
        {/* Home Bar */}
        <div className="h-1.5 w-32 bg-white/20 rounded-circle position-absolute bottom-2 left-1/2 -translate-x-1/2" />
      </div>
    </div>
  );

  return (
    <div className="w-100 h-100 d-flex flex-column overflow-hidden bg-brand-bg/30">
      <div className="flex-fill d-flex overflow-hidden">
        {/* Main Content Area */}
        <div className="flex-fill overflow-y-auto p-4 lg:p-8 d-flex flex-column align-items-center no-scrollbar">
          {/* Tab Switcher */}
          <div className="bg-white p-1 border border-brand-border d-flex gap-1 mb-8 shadow-sm flex-shrink-0 z-10" style={{'borderRadius':'20px'}}>
            {[
          ...
          {activeTab === 'preview' ? (
            ...
            <div className="w-100 max-w-4xl space-y-4 pb-20">
              {/* Form Name Card */}
              <div className="bg-white border border-brand-border p-6 lg:p-8 shadow-sm group focus-within:ring-2 focus-within:ring-brand-blue/10 transition-all mb-4" style={{'borderRadius':'24px'}}>
                <label className="fw-bold text-brand-gray text-uppercase mb-3 d-block opacity-60" style={{'fontSize':'10px', 'letterSpacing':'0.2em'}}>
                  Form Name
                </label>
                <input 
                  type="text" 
                  ...
                  className="w-100 text-xl fw-bold text-brand-dark bg-brand-bg/30 border border-brand-border rounded-3 px-4 py-3 focus:outline-none focus:ring-1 focus:ring-brand-blue/20 focus:bg-white transition-all placeholder:text-brand-gray/30"
                />
              </div>
                      ...
                      >
                        <div className="p-6 lg:p-8">
                          <div className="d-flex align-items-start gap-4">
                            <div className="mt-4 d-none sm:flex flex-column gap-1 cursor-grab active:cursor-grabbing text-brand-gray/20 group-hover:text-brand-gray/40 transition-colors">
                              <GripVertical size={20} />
                            </div>
                            
                            <div className="flex-fill space-y-6">
                              <div className="d-flex flex-column lg:flex-row lg:items-center justify-content-between gap-4">
                                ...
                                <div className="d-flex align-items-center gap-2 align-self-end lg:self-auto">
                                  ...
                                  <div className="position-relative group/type-menu">
                                    <button className="d-flex align-items-center gap-2 px-4 py-2.5 bg-brand-bg border border-brand-border rounded-3 fw-bold text-uppercase tracking-widest text-brand-gray hover:bg-white transition-all" style={{'fontSize':'10px'}}>
                                    ...
                                    <div className="position-absolute end-0 top-full mt-2 w-56 bg-white rounded-3 shadow-xl border border-brand-border opacity-0 invisible group-hover/type-menu:opacity-100 group-hover/type-menu:visible transition-all duration-200">
                              ...
                              <div className="py-2">
                                   ...
                                   <div className="space-y-3">
                                     <div className="d-flex gap-1">
                                       {Array.from({ length: (q.properties?.range || 10) + 1 }).map((_, i) => (
                                         <div key={i} className="flex-fill h-10 bg-brand-bg rounded-3 border border-brand-border d-flex align-items-center justify-content-center fw-bold text-brand-gray/40" style={{'fontSize':'10px'}}>{i}</div>
                                       ))}
                                     </div>
                                 ...
                                 {q.type === 'likert' && (
                                   <div className="space-y-3">
                                     <div className="d-flex gap-2">
                                       {Array.from({ length: q.properties?.range || 5 }).map((_, i) => (
                                         <div key={i} className="flex-fill h-12 bg-brand-bg rounded-3 border border-brand-border d-flex align-items-center justify-content-center fw-bold text-brand-gray/40" style={{'fontSize':'10px'}}>{i + 1}</div>
                                       ))}
                                     </div>
                                     <div className="d-flex justify-content-between px-1">
                                 ...
                                 {(q.type === 'radio' || q.type === 'checkbox') && (
                                   <div className="space-y-2">
                                     {(q.options || ['Option 1', 'Option 2']).map((opt, i) => (
                                       <div key={i} className="d-flex align-items-center gap-3 animate-in fade-in slide-in-from-left-2" style={{ animationDelay: `${i * 50}ms` }}>
                                 ...
                                 {q.type === 'text' && (
                                   <div className="h-10 w-100 bg-brand-bg/50 border border-dashed border-brand-border rounded-3 d-flex align-items-center px-4 fw-bold text-brand-gray/30 text-uppercase tracking-widest" style={{'fontSize':'10px'}}>Short answer text...</div>
                                 )}
                                 {q.type === 'paragraph' && (
                                   <div className="h-20 w-100 bg-brand-bg/50 border border-dashed border-brand-border rounded-3 d-flex align-items-center px-4 py-3 fw-bold text-brand-gray/30 text-uppercase tracking-widest align-items-start" style={{'fontSize':'10px'}}>Long answer text...</div>
                                 )}
                              </div>
                              ...
                              {isActive && (
                                <Motion.div 
                                  ...
                                  className="pt-6 mt-4 border-top border-brand-border d-flex align-items-center justify-content-between overflow-hidden"
                                >
                                  <div className="d-flex align-items-center gap-4 lg:gap-6">
                                    <button className="d-flex align-items-center gap-2 fw-bold text-brand-gray hover:text-brand-blue transition-colors text-uppercase tracking-widest" style={{'fontSize':'10px'}}>
                                    ...
                                    <button 
                                      ...
                                      className="d-flex align-items-center gap-2 fw-bold text-red-500 hover:text-red-600 transition-colors text-uppercase tracking-widest" style={{'fontSize':'10px'}}
                                    >
        ...
        <aside className={cn(
          ...
          <div className="p-6 border-bottom border-brand-border bg-white/50">
            <h3 className="text-xs fw-bold text-brand-dark text-uppercase tracking-widest d-flex align-items-center gap-2">
          ...
          <div className="flex-fill overflow-y-auto p-6 space-y-8 scrollbar-hide">
            {selectedQuestion ? (
              <>
                <div className="space-y-4">
                  <label className="fw-bold text-brand-gray text-uppercase tracking-widest d-block opacity-60" style={{'fontSize':'10px'}}>Question Text</label>
                  <textarea 
                ...
                {(selectedQuestion.type === 'rating' || selectedQuestion.type === 'nps' || selectedQuestion.type === 'likert') && (
                  <div className="space-y-4">
                    <label className="fw-bold text-brand-gray text-uppercase tracking-widest d-block opacity-60" style={{'fontSize':'10px'}}>Range Max</label>
                    <div className="position-relative">
                      <select 
                        ...
                        className="w-100 bg-brand-bg/50 border border-brand-border rounded-3 px-4 py-3 text-sm fw-bold text-brand-dark appearance-none outline-none focus:ring-1 focus:ring-brand-blue/30 transition-all cursor-pointer"
                      >
                ...
                {(selectedQuestion.type === 'radio' || selectedQuestion.type === 'checkbox') && (
                  <div className="space-y-4">
                    <div className="d-flex align-items-center justify-content-between mb-2">
                      <label className="fw-bold text-brand-gray text-uppercase tracking-widest d-block opacity-60" style={{'fontSize':'10px'}}>Choices</label>
                      <button 
                        onClick={() => {
                    ...
                    <div className="space-y-2">
                       {(selectedQuestion.options || []).map((opt, i) => (
                         <div key={i} className="d-flex align-items-center gap-2 group/choice animate-in slide-in-from-right-2" style={{ animationDelay: `${i * 50}ms` }}>
                           <input 
                             type="text"
                             value={opt}
                             onChange={(e) => {
                             ...
                             className="flex-fill bg-brand-bg/30 border border-brand-border rounded-3 px-3 py-2 text-xs fw-bold text-brand-dark focus:bg-white transition-all outline-none"
                           />
                           <button 
                             onClick={() => {
              ...
              <div className="d-flex flex-column align-items-center justify-content-center h-100 text-center py-20 px-8">
          ...
          <div className="p-6 border-top border-brand-border bg-white d-flex flex-column gap-3">
            <button 
              onClick={() => {
                toast.success('Opening Advanced Settings...');
              }}
              className="w-100 py-3 border border-brand-border rounded-3 fw-bold text-uppercase tracking-widest text-brand-gray hover:bg-brand-bg hover:text-brand-dark transition-all d-flex align-items-center justify-content-center gap-2" style={{'fontSize':'10px'}}
            >
              <Settings size={14} />
              Advanced Config
            </button>
            <div className="d-flex gap-3">
              <button 
                onClick={onBack}
                className="flex-fill py-3 border border-brand-border rounded-3 fw-bold text-uppercase tracking-widest text-brand-gray hover:bg-brand-bg transition-all" style={{'fontSize':'10px'}}
              >
                Cancel
              </button>
              <button 
                ...
                className="flex-fill py-3 bg-brand-blue text-white rounded-3 fw-bold text-uppercase tracking-widest shadow-lg shadow-brand-blue/30 hover:bg-brand-blue/90 transition-all d-flex align-items-center justify-content-center gap-2" style={{'fontSize':'10px'}}
              >
                <Check size={14} />
                Save Form
              </button>
